...
const SearchPage = () => {
  ...
  const {loading,searchRestaurant,searchedRestaurant,setAppliedFilter,appliedFilter} = useRestaurantStore()
    useEffect(()=>{
      // keep input in sync with route and trigger search
    ...
    <div className="max-w-7xl mx-auto my-10">
      <div className="flex flex-col md:flex-row justify-between gap-10 ">
        ...
        <div className="flex-1">
          ...
          <Button onClick={()=>navigate(`/search/${searchQuery}`)}  className={"!bg-orange-500"}>submit</Button>
          </div>
          {/* search items display */}
          <div>
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:gap-2 my-3">
              <h1 className="font-medium text-lg">({searchedRestaurant?.data?.length ?? 0}) search result found</h1>
              <div className="" >
                
                {appliedFilter.map((selectedFilter:string,idx:number)=>(
                    <div key={idx} className="relative inline-flex items-center max-w-full">
                            <Badge className="!bg-gray-600 !text-[#D19254] rounded-md hover:cursor-pointer pr-6  whitespace-nowrap" variant={"outline"}>{selectedFilter}</Badge>
                            <X
            ...
            <div className="grid md:grid-cols-3 gap-3">
                {
                    loading? <SearchPageSkeleton/>:(
                      !loading && (searchedRestaurant?.data?.length ?? 0) === 0 ?( <NoResultFound searchText={params.text!}/>)
                      :
                      (
                  ...
                  <div className="flex gap-2 mt-4 flex-wrap">
                    {
                      restaurant.cuisines.map((cuisine:string,idx:number)=>(
                        <Badge key={idx}
                        className="font-medium px-2 py-1 rounded-full shadow-sm">{cuisine}</Badge>
                      ))
                    }
                  </div>
                </CardContent>
                ...
                <Button className='!bg-red font-semibold py-2 px-4 rounded-full shadow-md transition-colors duration-150'>View menu</Button>
                </Link>
                </CardFooter>
              </Card>
                  ))
